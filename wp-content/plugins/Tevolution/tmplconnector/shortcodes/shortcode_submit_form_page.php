 ...
 * Return: display the breadcrum as per submit.edit and delete submit page.
 */
...
function tevolution_form_page_template($atts){
	...
	{
		echo '<span class="message_error2">'.__("The tevolution post type and tevolution submit form shortcode post type doesn't match. Please select the same post type.").'</span>';
		return;
	}
	...
	if((isset($_REQUEST['category']) && count($_REQUEST['category']) > 0 ) || $tmpdata['templatic-category_custom_fields'] == 'No' || $_REQUEST['fields']){
		...
		if(is_plugin_active('wpml-translation-management/plugin.php')){
			...
			if(isset($_REQUEST['lang'])){
				...
				if(strpos(get_permalink($post->ID),$sitepress->get_current_language()) == false)
		...
		/* Edit Form Security Code */
			...
			{ 
				...
				?>
                        ...
                        <?php else:?>
                              <input type="submit" name="preview" value="<?php  _e('Preview ',DOMAIN); echo ucfirst(get_post_meta($post_id,'submit_post_type',true));?>" class="normal_button main_btn" <?php echo $submit_button; ?>/>    
                        <?php endif;?>
