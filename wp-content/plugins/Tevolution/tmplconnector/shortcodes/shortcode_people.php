...
function tevolution_author_list_fun($atts){
       ...
       <ul class="peoplelisting">
		...
		foreach($arrpeoples as $key => $value)
			{
			 $userDetail=get_user_meta( $value->ID,'user_address_info'); ?>
                 ...
                 <?php if(get_user_meta($value->ID,'profile_photo',true) != ""){
						...
						if(tevolution_get_posts_count($value->ID) > 1)
						{
							$total_entries = __('Total entries',DOMAIN);
						}
						elseif(tevolution_get_posts_count($value->ID) == 1)
						{
							$total_entries = __('Total entry',DOMAIN);
						}
						else
						{
							$total_entries = __('No entries',DOMAIN);
						}
			    ?> 
                   <div class="people_info">    
                	 <h3><span class="fl"> <a href="<?php echo get_author_posts_url($value->ID);?>"><?php echo $value->display_name; ?></a> </span><span class="total_homes"> <?php echo $total_entries; ?> <a href="<?php echo get_author_posts_url($value->ID);?>"><?php echo tevolution_get_posts_count($value->ID); ?></a></span></h3>
                     <p class="peoplelink" >
		...
		{
		?>
		<p class="ac"><?php _e('Whoops, looks like there is no registered user whose name starts with alphabet ',DOMAIN);?><b><?php echo strtoupper($kw);?>.</b></p>
		<?php
		}
		$paged = (get_query_var('paged')) ? get_query_var('paged') : 1;
