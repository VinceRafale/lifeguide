...
if($activated){
templatic_module_activationmsg('templatic-login','User registration/Login Management','',$mod_message=__('You can use this feature directly by setting up a widget T - Login Dashboard wizard in your site',DOMAIN).' <a href='.admin_url('/widgets.php').'><strong>'.__('from here',DOMAIN).'</strong></a>. '.__('On activation of this module, it will automatically add two user custom fields on your site. Add some more',DOMAIN).' <a href='.admin_url('/admin.php?page=user_custom_fields').'><strong>'.__('from here',DOMAIN).'</strong></a>.',$realted_mod =''); 
}else{
...
<div id="templatic_userreg" class="widget_div">
    ...
    <div class="inside">
        ...
        <a href="<?php echo site_url(); ?>/wp-admin/admin.php?page=user_custom_fields"><h3 class="hndle"><span><?php _e('User registration/Login Management',DOMAIN); ?></span></h3></a>
		<p class="mod_desc"><?php
		_e('Activate this module if you plan on allowing visitors to login and post content on your site. Use the "Profile Fields Setup" section to create new fields for the registration form and collect unique information about your visitors.',DOMAIN);
		?></p>
        </div>
		<?php if(!is_active_addons('templatic-login')) { ?>
		
		<div id="publishing-action" class="settings_style">
		<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&activated=templatic-login&true=1";?>" class="button-primary"><i class="fa fa-check"></i>&nbsp;<?php _e('Activate &rarr;',DOMAIN); ?></a></div>
		<?php } 
		...
		<div id="publishing-action" class="settings_style">
			...
			<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&deactivate=templatic-login&true=0";?>" class="button"><i class="fa fa-times"></i>&nbsp;<?php _e('Deactivate ',DOMAIN); ?></a> 		
		<?php 
			global $wpdb;
