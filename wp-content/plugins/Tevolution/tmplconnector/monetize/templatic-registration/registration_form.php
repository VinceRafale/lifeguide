	...
	function tevolution_register_user($user_login, $user_email){
		...
		if( $tmpdata['recaptcha'] == 'recaptcha')
		{
			...
			if(file_exists(get_tmpl_plugin_directory().'wp-recaptcha/recaptchalib.php') && is_plugin_active('wp-recaptcha/wp-recaptcha.php') && in_array('registration',$display))
			{
				...
				if (!$resp->is_valid ) {
					$errors->add('captcha', sprintf(__('ERROR: %s',DOMAIN),INVALIDCAPTCHA));
				}
			}
		}
		...
		if ( $user_id ) {
			...
			{
				$login_url_link=get_tevolution_login_permalink().'&akey='.$activation_key.'&uid='.base64_encode($user_id);
			}else{
				$login_url_link=get_tevolution_login_permalink().'?akey='.$activation_key.'&uid='.base64_encode($user_id);
			}
...
<div id="sign_up">  
  <div class="registration_form_box">
    <h3><?php _e(REGISTRATION_NOW_TEXT,DOMAIN); ?> </h3>
    <?php
