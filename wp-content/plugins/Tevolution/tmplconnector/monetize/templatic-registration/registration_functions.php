	...
	if(isset($author) && $author !='') :
		  ...
		  if($current_user->ID == $curauth->ID)
		  {
		   ...
		   ?>
               <div class="author_social_networks social_media">
                   ...
                   <?php if($facebook!=''):?>
                    <li><a href="<?php echo (strpos('http://',$facebook) !== false)?$facebook:'http://'.$facebook; ?>" target="_blank"><abbr title="<?php _e("Facebook",DOMAIN);?>">F</abbr></a></li>
                    <?php endif;?>
                    <?php if($twitter):?>
                    <li><a href="<?php echo (strpos('http://',$twitter) !== false)?$twitter:'http://'.$twitter; ?>" target="_blank"><abbr class="twitter">t</abbr></a></li>
                    <?php endif;?>
                    <?php if($linkedin):?>
                    <li><a href="<?php echo (strpos('http://',$linkedin) !== false)?$linkedin:'http://'.$linkedin; ?>" target="_blank"><abbr class="linkedin">l</abbr></a></li>
                    <?php endif;?>
                   </ul>
               </div>
          <?php endif;?>
		...
		$posttaxonomy = get_option("templatic_custom_post");
			...
			if($pkg_type == 2 && $current_user->ID != ''){
				...
				if($remaining >0 ){
					echo apply_filters('author_subscription_message',sprintf(__('You have subscribed to %s price package for %s. <br/>Total number of posts: %s, Submited: %s, Remaining: %s',DOMAIN),'<b>'.$package_name.'</b>','<b>'.rtrim($pkg_post_type1,',').'</b>','<b>'.$limit_no_post.'</b>','<b>'.$submited.'</b>','<b>'.$remaining.'</b>'));	
				}else{
					echo apply_filters('author_subscription_message',sprintf(__('You have subscribed to %s price package for %s. <br/> Total number of posts: %s',DOMAIN),'<b>'.$package_name.'</b>','<b>'.rtrim($pkg_post_type1,',').'</b>','<b>'.$limit_no_post.'</b>'));
				}			
				echo ".</div>";
...
function templ_fecth_login_onsubmit(){ ?>
	<div class="login_submit clearfix" id="loginform">
                  <div class="sec_title">
                  	<h3 class="form_title spacer_none"><?php _e(LOGINORREGISTER_PLUGIN,DOMAIN);?></h3>
                  </div>
					<?php 
                  ...
                  <form name="loginform" class="sublog_login" <?php if($user_login_or_not=='existing_user' || $user_login_or_not == '' ){ ?> style="display:block;" <?php } else {  ?> style="display:none;" <?php }?>  id="login_user_frm_id" action="<?php echo get_settings('home').'/index.php?page=login'; ?>" method="post" >
        ...
        if($val['type']=='radio')
        {
            ...
            if($options)
            {
                ...
                for($i=0;$i<count($option_values_arr);$i++)
                {
                    ...
                    $str .= '<div class="form_cat">'.$val['tag_before'].'<input name="'.$key.'" type="radio" '.$val['extra'].'  value="'.$option_values_arr[$i].'" '.$seled.'> '.$option_values_arr[$i].$val['tag_after'].'</div>';
                }
                if($val['is_require'])
                {
                    $str .= '<span id="'.$key.'_error"></span>';	
