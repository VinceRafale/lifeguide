...
{ 
	$menu_title1 = __('Profile Fields Setup',DOMAIN);
...
function tmpl_reg_js(){
	...
	if($post->ID == $login_page_id || $post->ID==$register_page_id || $post->ID==$profile_page_id || $is_tevolution_submit_form==1){
...
function registration_email_setting($tabs ) {			
	$tabs['email']=__('Email Settings',DOMAIN);
	return $tabs;
}	
/*
	...
	switch($column)
	{
		case 'email':	
			...
			<tr class="registration-email alternate">
				<td><label class="form-textfield-label"><?php _e('After registration email',DOMAIN); ?></label></td>
			   
				<td>
					<a href="javascript:void(0);" onclick="open_quick_edit('registration-email','edit-registration-email')"><?php _e("Quick Edit",DOMAIN);?></a> 
					| 
					<a href="javascript:void(0);" onclick="reset_to_default('registration_success_email_subject','registration_success_email_content','registration-email');"><?php _e("Reset",DOMAIN);?></a>
						...
						<tr>
							<td colspan="2">
								<div class="buttons">
									...
									<a class="button-secondary cancel alignright " href="javascript:void(0);" onclick="open_quick_edit('edit-registration-email','registration-email')" accesskey="c">Cancel</a>
...
function templatic_general_setting_register_data($column){
	...
	?>
		<tr id="registration_page_setup">
			<th colspan="2"><div class="tevo_sub_title"><?php _e('Registration options',DOMAIN);?></div> <p class="tevolution_desc"><?php _e('Match your Login, Register and Profile pages below to ensure registration works correctly. These pages were created automatically when Tevolution was activated. If you need to create them manually please open the <a href="http://templatic.com/docs/tevolution-guide/#registration" target= "_blank"> documentation guide</a>',DOMAIN)?></p></th>
		</tr>
		 <tr>
			<th><label><?php _e('Allow user to auto login after registration',DOMAIN);  ?></label></th>
			<td>
			   <div class="input_wrap"><label for="allow_autologin_after_reg"><input type="checkbox" id="allow_autologin_after_reg" name="allow_autologin_after_reg" value="1" <?php if(isset($tmpdata['allow_autologin_after_reg']) && $tmpdata['allow_autologin_after_reg']==1){?>checked="checked"<?php }?> />&nbsp;<?php _e('Enable',DOMAIN);?></label></div>
				<p class="description"><?php _e('Enabling this option will automatically show the user status as logged in after registering on your site.',DOMAIN); ?></p>
			</td>
		 </tr>  
		 <tr>
			<th><label><?php _e('Login Page',DOMAIN);?></label></th>
			<td>
				<?php $pages = get_pages();?>
				<select id="tevolution_login" name="tevolution_login">
					...
					$select_page=$tmpdata['tevolution_login'];
						...
						echo '<option>' . __('No pages found', DOMAIN) . '</option>';
					endif;
					?>
				</select> 
				<div style="display:none" id="tevolution_login_page" class="description act_success  tevolution_highlight"><?php _e("Copy this shortcode and paste it in the editor of your selected page to make it work correctly.<br> Shortcode - [tevolution_login] (including square braces)"); ?></div>
			</td>
		 </tr>
		  <tr>
			<th><label><?php _e('Register Page',DOMAIN);?></label></th>
			<td>
				<?php $pages = get_pages();?>
				<select id="tevolution_register" name="tevolution_register">
					...
					?>
				</select> <div style="display:none" id="tevolution_register_page" class="description act_success  tevolution_highlight"><?php _e("Copy this shortcode and paste it in the editor of your selected page to make it work correctly.<br> Shortcode - [tevolution_register] (including square braces)"); ?></div>
			</td>
		 </tr>
		  <tr>
			<th><label><?php _e('Profile Page',DOMAIN);?></label></th>
			<td>
				<?php $pages = get_pages();?>
				<select id="tevolution_profile" name="tevolution_profile">
					...
					?>
				</select> <div style="display:none" id="tevolution_profile_page" class="description act_success tevolution_highlight"><?php _e("Copy this shortcode and paste it in the editor of your selected page to make it work correctly.<br> Shortcode - [tevolution_profile] (including square braces)"); ?></div>
			</td>
		 </tr>
	<?php			
}
