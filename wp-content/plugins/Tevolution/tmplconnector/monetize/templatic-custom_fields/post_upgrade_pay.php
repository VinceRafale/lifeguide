...
if($_POST){
	...
	if($_POST['paynow']){
		...
		///////ADMIN EMAIL START//////
			...
			$email_subject = __(sprintf('A New Upgrade Request of ID:#%s',$post_id));
		    ...
		    $email_subject_user = __(sprintf('Payment Pending For Upgrade Request: #%s',$post_id));
			...
			if($_REQUEST['pid'])
			{
				$email_content_user = __(sprintf('<p>Dear [#to_name#],</p><p>Your %s has been updated by you . Here is the information about the %s:</p>[#information_details#]<br><p>[#site_name#]</p>',$mail_post_title,$mail_post_title));
			}
			if(isset($_SESSION['upgrade_post']['renew']))
			{
				$email_content_user = __(sprintf('<p>Dear [#to_name#],</p><p>Your %s has been renew by you . Here is the information about the %s:</p>[#information_details#]<br><p>[#site_name#]</p>',$mail_post_title,$mail_post_title));
				
			}	
			$information_details = "<p>".__('ID')." : ".$post_id."</p>";
			$information_details .= '<p>'.__('View more detail of').' <a href="'.get_permalink($post_id).'">'.stripslashes($my_post['post_title']).'</a></p>';
			...
			if(is_active_addons('monetization') && $payable_amount > 0){
				...
				$information_details .= '<p>'.__('Payment Method: <b>'.ucfirst(@$_POST['paymentmethod']).'</b>').'</p>';
			}else{
