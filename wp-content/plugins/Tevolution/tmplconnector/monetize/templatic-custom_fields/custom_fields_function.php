...
function get_search_post_fields_templ_plugin($post_types,$category_id='',$taxonomy='') {
	...
	$post_query = get_transient( '_tevolution_query_search_'.trim($post_types) );
	if ( false === $post_query && get_option('tevolution_cache_disable')==1 ) {
		$post_query = new WP_Query($args);
		set_transient( '_tevolution_query_search_'.trim($post_types), $post_query, 12 * HOUR_IN_SECONDS );		
...
function display_search_custom_post_field_plugin($custom_metaboxes,$session_variable,$post_type){
		...
		<div class="form_row clearfix">
		   ...
		   <?php if($name == 'geo_latitude' || $name == 'geo_longitude') {
			...
			?>			
			<textarea PLACEHOLDER="<?php echo $site_title; ?>" name="<?php echo $name;?>" id="<?php echo $name;?>" class="<?php if($style_class != '') { echo $style_class;}?> textarea" <?php echo $extra_parameter;?>><?php if(isset($value))echo $value;?></textarea>       
			<?php
			}elseif($type=='radio'){
			?>
			...
			elseif(!isset($_REQUEST['action']) && $type=='post_categories' && @$tmpdata['templatic-category_custom_fields'] == 'No')
				...
				<div class="form_row clearfix">
						...
						<label><?php echo $site_title; ?></label>
						 ...
						 <span class="message_note msgcat"><?php echo sprintf(__('Select at least one category for your %s',DOMAIN),$post_type);?></span>
	...
	?>
	   ...
	   <?php if($type=='post_categories')
					...
					if($category_name)
					 {
						...
						echo '<input type="hidden"  value="'.$value.'" name="category_new[]">';
					 }
				}
		...
		if(strpos($permalink,'?'))
		{
			...
			?>
			  <a href="<?php echo $gobacklink; ?>" class="btn_input_normal fl" ><?php echo __(GO_BACK_AND_EDIT_TEXT,DOMAIN);?></a>
			<?php } ?>
...
<p class="meta-options">
	<label for="comment_status" class="selectit"><input name="comment_status" type="checkbox" id="comment_status" value="open" <?php checked($post->comment_status, 'open'); ?> /> <?php _e( 'Allow reviews.' ) ?></label><br />
...
function tevolution_comment_meta_box( $post ) {
	...
	?>
	<p class="hide-if-no-js" id="add-new-comment"><a href="#commentstatusdiv" onclick="commentReply.addcomment(<?php echo $post->ID; ?>);return false;"><?php _e('Add reviews'); ?></a></p>
	<?php
	$total = get_comments( array( 'post_id' => $post->ID, 'number' => 1, 'count' => true ) );
	$wp_list_table = _get_list_table('WP_Post_Comments_List_Table');
	$wp_list_table->display( true );
	if ( 1 > $total ) {
		echo '<p id="no-comments">' . __('No reviews yet.') . '</p>';
	} else {
		...
		?>
		<p class="hide-if-no-js" id="show-comments"><a href="#commentstatusdiv" onclick="commentsBox.get(<?php echo $total; ?>);return false;"><?php _e('Show reviews'); ?></a> <span class="spinner"></span></p>
		<?php
	}
		...
		if($pagenow=='post.php' || $pagenow=='post-new.php'){			
			...
			foreach($custom_post_types as $post_type => $value){
				if($posttype==$post_type){
					...
					add_meta_box('commentstatusdiv', __('Review Settings'), 'tevolution_comment_status_meta_box', $post_type, 'normal', 'low');
					...
					if ( ( 'publish' == get_post_status( @$_REQUEST['post'] ) || 'private' == get_post_status( @$_REQUEST['post'] ) ) && post_type_supports($post_type, 'comments') ){
						...
						add_meta_box('commentsdiv', __('Reviews'), 'tevolution_comment_meta_box', $post_type, 'normal', 'low');
					}
					...
					foreach($heading_type as $key=>$val){
						$meta_name=($val=='[#taxonomy_name#]')? sprintf(__('%s Information',DOMAIN),$new_menu_name) : sprintf(__('%1$s',DOMAIN),$val);
						add_meta_box('ptthemes-settings'.$key,$meta_name,'tvolution_custom_meta_box_content',$post_type,'normal','high',array( 'post_types' => $post_type,'heading_type'=>$val));
					}
					   ...
					   if(is_active_addons('monetization')){
					    global $monetization;
							...
							if(count($package_query->posts) != 0)
								add_meta_box('ptthemes-settings-price-package',__('Price Packages',DOMAIN),'tevolution_featured_list_fn',$post_type,'normal','high',array( 'post_types' => $post_type));			
					
						
					}
					add_meta_box( 'ptthemes-settings-image-gallery', __( 'Image Gallery', DOMAIN ), 'tevolution_images_box', $post_type, 'side','',$post );
				}
	   ...
	   foreach ($pt_metaboxes as $pt_id => $pt_metabox) {
		if($pt_metabox['type'] == 'text' OR $pt_metabox['type'] == 'select' OR $pt_metabox['type'] == 'radio' OR $pt_metabox['type'] == 'checkbox' OR $pt_metabox['type'] == 'textarea' OR $pt_metabox['type'] == 'upload' OR $pt_metabox['type'] == 'date' OR $pt_metabox['type'] == 'multicheckbox' OR $pt_metabox['type'] == 'texteditor')
				...
				elseif ($pt_metabox['type'] == 'textarea'){
					...
					echo "<td>";
					do_action('tmpl_custom_fields_'.$pt_metabox["name"].'_before');
					echo  '<textarea class="pt_input_textarea" name="'.$pt_metabox["name"].'" id="'.$pt_id.'">' . $pt_metaboxvalue . '</textarea>';
					do_action('tmpl_custom_fields_'.$pt_metabox["name"].'_after');
					echo  '<p class="description">'.$pt_metabox['desc'].'</p>';
					echo  "</td></tr>";
...
function pagenavi_plugin($before = '', $after = '') {
   ...
   // $pagenavi_options['pages_text'] = ('Page %CURRENT_PAGE% of %TOTAL_PAGES%:');
    ...
    $pagenavi_options['next_text'] = '<strong class="next page-numbers">'.__('NEXT',DOMAIN).'</strong>';
    $pagenavi_options['prev_text'] = '<strong class="prev page-numbers">'.__('PREV',DOMAIN).'</strong>';
		...
		$active_languages = $sitepress->get_active_languages();
			...
			if($v['code']==$sitepress->get_current_language()) continue;
			 ...
			 if(isset($__management_columns_posts_translations[$id][$v['code']]) && $__management_columns_posts_translations[$id][$v['code']]->element_id){
				 ...
				 $alt = sprintf(__('Edit the %s translation',DOMAIN), $v['display_name']);				 
				...
				$alt = sprintf(__('Add translation to %s',DOMAIN), $v['display_name']);
		...
		if($_SESSION["file_info"])
		{
		 ...
		 ?>
             ...
             <div class="submit_info_section">
                <h3><?php _e(MORE_PHOTOS .' ' . $single_gallery_post_type,DOMAIN) ?></h3>
              </div>
             <div id="gallery">
...
function templ_preview_page_file_upload_display()
{
	...
	if(isset($_SESSION['upload_file']) && $_SESSION['upload_file'] != ""):
	   foreach($_SESSION['upload_file'] as $fileval):
		if($fileval):
	?>
		  <p><?php _e('Click here to download File'); ?> <a href="<?php echo $fileval; ?>" class="normal_button main_btn"><?php _e('Download',DOMAIN); ?></a></p>
	<?php
		endif;
	   endforeach;
	endif;	
...
{	
	...
	{		
		...
		{
			add_filter('posts_where', 'event_manager_posts_where',11);
...
add_action('init','tev_transaction_msg');
	...
	if(count($_REQUEST['cf'])>0)
	{
		for($i=0;$i<count($_REQUEST['cf']);$i++)
		{
			...
			if(isset($_REQUEST['action']) && $_REQUEST['action'] !='' && $_REQUEST['action'] !='delete'){
				...
				$transaction_details .= "Payment Details for $post_name <br/>\r\n";
				$transaction_details .= "-------------------------------------------------- <br/>\r\n";
...
function tevolution_submition_success_msg_fn(){
	...
	$fetch_status = $wpdb->get_var("select status from $transaction where post_id=$orderId");
	...
	if($fetch_status)
	{
		$filecontent = sprintf(__('<p class="sucess_msg_prop">Thank you for your submission. Your %s has been successfully received.</p>[#submited_information_link#]<p></p>',DOMAIN),$post_lable); 
	}
					...
					elseif(get_post_meta($post->ID,"ctype",true) == 'radio')
					{
					...
					}else
					 {
						 ...
						 else
						 {
						   if(get_post_meta($post->ID,'ctype',true) == 'upload')
							{
							  echo "<li><p class='submit_info_label'>".stripslashes($post->post_title).": </p> <p class='submit_info_detail'> Click here to download File <a href=".get_post_meta($_REQUEST['pid'],$post->post_name,true).">Download</a></p></li>";
							}
						   else
							{
...
<?php
			...
			{
				 echo "<li><p class='submit_info_label'>".__('Validity',DOMAIN).": </p> <p class='submit_info_detail'> ".get_post_meta($_REQUEST['pid'],'alive_days',true).' '.__('Days',DOMAIN)."</p></li>";
			}
			if(get_user_meta($suc_post->post_author,'list_of_post',true))
			{
				 echo "<li><p class='submit_info_label'>".__('Number of Posts',DOMAIN).": </p> <p class='submit_info_detail'> ".get_user_meta($suc_post->post_author,'list_of_post',true)."</p></li>";
			}
			if(get_post_meta(get_post_meta($_REQUEST['pid'],'package_select',true),'recurring',true))
			{
				$package_name = get_post(get_post_meta($_REQUEST['pid'],'package_select',true));
				 echo "<li><p class='submit_info_label'>".__('Recurring Charges',DOMAIN).": </p> <p class='submit_info_detail'> ".fetch_currency_with_position(get_post_meta($_REQUEST['pid'],'paid_amount',true))."</p></li>";
			}
...
add_filter('body_class','directory_favourites_class',10,2);
...
function tevolution_images_box($post){
	...
	<div id="images_gallery_container">
     ...
     <p class="add_tevolution_images hide-if-no-js">
		<a href="#"><?php _e( 'Add images gallery', DOMAIN ); ?></a>
	</p>
     <p class="description"><?php _e('<b>Note:</b> You cannot directly select the images from the media library, instead you have to upload a new image.',DOMAIN);?></p>
     </div>
	<script type="text/javascript">
		jQuery(document).ready(function($){
			// Uploading files
			...
			jQuery('.add_tevolution_images').on( 'click', 'a', function( event ) {
				...
				image_gallery_frame = wp.media.frames.downloadable_file = wp.media({
					...
					title: '<?php _e( 'Add images gallery', DOMAIN ); ?>',
					button: {
						text: '<?php _e( 'Add to gallery', DOMAIN ); ?>',
					},
					multiple: true
				});
						...
						if ( attachment.id ) {
							...
							$images_gallery.append('\
								<li class="image" data-attachment_id="' + attachment.id + '">\
									...
									<ul class="actions">\
										<li><a href="#" class="delete" title="<?php _e( 'Delete image', DOMAIN ); ?>"><?php _e( 'Delete', DOMAIN); ?></a></li>\
									</ul>\
								</li>');
						}
					} );
			...
			jQuery('#images_gallery_container').on( 'click', 'a.delete', function() {
				...
				if(delete_id!=''){
					jQuery.ajax({
						url:"<?php echo js_escape( get_bloginfo( 'wpurl' ) . '/wp-admin/admin-ajax.php' ); ?>",
						type:'POST',
