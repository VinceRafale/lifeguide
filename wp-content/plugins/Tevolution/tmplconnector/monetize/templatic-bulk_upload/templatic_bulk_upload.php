...
<div class="wrap">
	...
	<h2><?php _e('Bulk Import/Export',DOMAIN); ?></h2>
	<p class="tevolution_desc"><?php _e('Save your time by directly performing Import/export process here (it only accepts .csv format). You can import/export thousands of records of your desired post type in no time.',DOMAIN);?><strong> <?php _e('Note',DOMAIN); ?>: </strong><?php _e('For a successful import/export you will have to follow the sample CSV format, there is a link below to download the sample file.',DOMAIN); ?></p>	
	...
	if(isset($_POST['start']) && $_POST['start']!="" && $_POST['start']!=0 && isset($_SESSION['csv_data']) && $_SESSION['csv_data']!=""){
		...
		echo '<div class="updated fade" style="padding:10px;margin:0 0 10px;">';
		_e("<b>$imported imported, $updated updated, $skipped skipped of $total_record posts. $completed</b>",DOMAIN);
		echo '</div>';
	}
	if(isset($_REQUEST['structure_error']) && $_REQUEST['structure_error']!="" && $_REQUEST['structure_error']==1){
		...
		$download = "<a href='".get_bloginfo("url")."/wp-admin/admin.php?page=bulk_upload&ptype=csvdl' style='color:#21759B'>download</a>";
          _e("csv file structure doesn't match. Please $download sample csv file to see required structure.",DOMAIN);
		echo '</div>';
	}
	do_action('tevolution_before_bulk_upload');?>
	<!-- It's section to export csv form BOF-->
	<div class="tevo_sub_title"><?php _e('CSV Import',DOMAIN); ?> </div>
	<form action="<?php echo site_url('/wp-admin/admin.php')?>?page=bulk_upload" method="post" name="bukl_upload_frm" enctype="multipart/form-data" id="bukl_upload_frm" onsubmit="return chek_file();">
	     <input type="hidden" name="ptype" id="ptype" value="post"/>
          <p class="tevolution_desc"><?php _e('Directly upload all the content of your desired custom post type (e.g. events, places, posts etc) here and check under its section located at your WordPress menu panel.(e.g. If you upload data for events then check under events section in your wp-admin panel)',DOMAIN)?></p>
          <table class="form-table">
               <tbody>
                    ...
                    <div id="csv_import_id">
                         ...
                         <br /><br/>
                         <input type="submit" class="button button-primary" name="submit" id="submit" value="<?php _e('Import csv',DOMAIN); ?>"/>
                    ...
                    <p class="description"><i><?php _e('Download the sample file to see the correct structure of the .csv file. To use bulk upload with custom fields simply add them as new columns inside the .csv file. Add them last (at the end).', DOMAIN);?> </i></p>
                    </td>
                    </tr>
               </tbody>
          </table>
	</form>
	<div class="tevo_sub_title"><?php _e('CSV Export',DOMAIN); ?></div>
	<table class="form-table">
		<tbody><p class="tevolution_desc"><?php _e('Directly download all the content of your desired custom post type (e.g. events, places, posts etc) from here and save it on your hard drive.',DOMAIN)?>
			<tr>
				<th><?php _e('Select post type',DOMAIN);?> </th>
			<td>
					...
					?>
                         <br/>     
                         <input type="submit" name="export_to_csv" value="<?php _e('Export TO csv',DOMAIN);?>" class="button button-primary" id="submit">
                         <p class="description"><i><?php _e('If your current theme doesn&lsquo;t support .csv format then follow <a href="http://codex.wordpress.org/Backing_Up_Your_Database">these</a> steps  to export your post from database.<p class="description"><b>Note</b>: Instead of exporting in sql format,just export them into .csv format</p>',DOMAIN);?></i></p>
                    </form>	
			</td>
			</tr>
		</tbody>
...
if($_POST && isset($_SESSION['file_name']) && $_SESSION['file_name']!=""){
	...
	for($i=$start;$i<$loop; $i++){
		...
		if($postid==""):
			$new_post = array(
			'post_title'   => convert_chars(addslashes(iconv('', 'utf-8',$_SESSION['csv_data'][$i]['templatic_post_title']))),
			'post_content' => wpautop(convert_chars(addslashes(iconv('', 'utf-8',$_SESSION['csv_data'][$i]['templatic_post_content'])))),
			'post_status'  => 'publish',	
			'post_type'    => $post_type,
			'post_date'    => date('Y-m-d H:i:s', strtotime($_SESSION['csv_data'][$i]['templatic_post_date'])),
			'post_excerpt' => convert_chars(addslashes(iconv('', 'utf-8',$_SESSION['csv_data'][$i]['templatic_post_excerpt']))),
			'post_name'    => convert_chars(addslashes(iconv('', 'utf-8',$_SESSION['csv_data'][$i]['templatic_post_name']))),
		 ...
		 elseif($postid!=""):	
			...
			$new_post = array('ID'=>$postid,
				...
				'post_author'  =>($_SESSION['csv_data'][$i]['templatic_post_author']) ? $_SESSION['csv_data'][$i]['templatic_post_author'] : 0,
				'post_parent'  => $_SESSION['csv_data'][$i]['templatic_post_parent'],
				'tags_input' => convert_chars(addslashes(iconv('', 'utf-8',$_SESSION['csv_data'][$i]['templatic_post_tag']))),
			 );
               ...
               foreach ($categories as $category) {
                   if ($category) {
                       ...
                       if ($term) {
                           ...
                           $term_id = wp_insert_category(array('cat_name' => $category,'category_parent' => $parent_id));
                           $ids['cleanup'][] = $term_id;
                       }
                       $parent_id = $term_id;
                   }
...
function create_templatic_custom_field($post_id, $data) {
	foreach ($data as $k => $v) {
		...
		if (!preg_match('/^templatic_/', $k) && $v != '') {
			...
			update_post_meta($post_id, trim($k), convert_chars(addslashes(iconv('', 'utf-8',$v))));
		}
	}
}
//
