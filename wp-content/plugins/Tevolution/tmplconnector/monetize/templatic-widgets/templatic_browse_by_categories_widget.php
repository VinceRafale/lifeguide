...
class templatic_browse_by_categories extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => 'widget Templatic browse_by_categories', 'description' => __('Display categories of a specific post type. Works best in sidebar areas.',DOMAIN) );
		$this->WP_Widget('templatic_browse_by_categories', __('T &rarr; Browse By Categories',DOMAIN), $widget_ops);
	}
	...
	// prints the widget
		...
		if ( false === ( $widget_category_list = get_transient( '_tevolution_query_browse_categories_'.$post_type.$transient_name) ) && get_option('tevolution_cache_disable')==1 ) {
			do_action('tevolution_category_query');
			$widget_category_list =  wp_list_categories($cat_args);
			set_transient( '_tevolution_query_browse_categories_'.$post_type.$transient_name, $widget_category_list, 12 * HOUR_IN_SECONDS );				
	...
	//widgetform in backend
		...
		$title = ($instance['title']) ? strip_tags($instance['title']) : __("Find By Category",DOMAIN);
    ...
    <p>
    	<label for="<?php echo $this->get_field_id('post_type');?>" ><?php _e('Select Post Type:',DOMAIN);?></label>	
    ...
    <?php
     ...
     <?php }?>	
    	...
    	<span><?php _e('Display all categories list from the selected post type.',DOMAIN);?></span>
    </p>
	<p>
	  ...
	  <label for="<?php echo $this->get_field_id('categories_count'); ?>"><?php _e('Show Categories Count:',DOMAIN);?></label>
	</p>	
	<?php
	}
}
