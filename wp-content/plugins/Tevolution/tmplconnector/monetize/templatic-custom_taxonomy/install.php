...
function templatic_extra_actions($actions, $post){
	...
	$sql = "select trans_id from $transaction_table where 1=1 and post_id=".$post->ID;
...
function create_custom_taxonomy() {
	...
	if($args):
		foreach($args as $key=> $_args)
		{
			...
			register_sidebars(1,array('id'=>''.$_args["labels"]["singular_name"].'_listing_sidebar','name'=> apply_filters('listing_page_sidebar_title',sprintf(__('%s Category Page Sidebar',DOMAIN),ucfirst($_name)),$_name),'description'=>sprintf(__('Display widgets in a sidebar on %s category pages.',DOMAIN),$_name),'before_widget'=>'<div id="%1$s" class="widget %2$s widget-%2$s">','after_widget'=>'</div>','before_title'=>'<h3>','after_title'=>'</h3>'));
			
			/*Single post Type sider bar  */
			register_sidebars(1,array('id'=>''.$_args["post_slug"].'_detail_sidebar','name'=>sprintf(__('%s Detail Page Sidebar',DOMAIN),ucfirst($_name)),'description'=>sprintf(__('Display widgets in a sidebar on single %s pages.',DOMAIN),$_name),'before_widget'=>'<div id="%1$s" class="widget %2$s widget-%2$s">','after_widget'=>'</div>','before_title'=>'<h3>','after_title'=>'</h3>'));
			
			/*Add post submit side bar*/
			register_sidebars(1,array('id'=>'add_'.$_args["post_slug"].'_submit_sidebar','name'=>sprintf(__('%s Add - Sidebar',DOMAIN),ucfirst($_name)),'description'=>sprintf(__('Display widgets in a sidebar that appears on %s submission pages.',DOMAIN),$_name),'before_widget'=>'<div id="%1$s" class="widget %2$s widget-%2$s">','after_widget'=>'</div>','before_title'=>'<h3>','after_title'=>'</h3>'));
		...
		$package_id=get_post_meta($post_id,'package_select',true);
			if($package_id!=""){
				if($post->post_parent)
					...
					$post_id = $post->ID;
				$transaction_table = $wpdb->prefix.'transactions';
				$sql = "select trans_id from $transaction_table where 1=1 and post_id=".$post_id;
				$trans_id = $wpdb->get_results($sql);
				if(!empty($trans_id) && $trans_id[0]->trans_id)
	...
	case 'categories_' :
		...
		case 'price_package' :
			...
			if($package_id!=""){
				...
				$publish_date =  date_i18n('Y-m-d',strtotime($trans_post->post_date));
				...
				if( 'h' == $featured_type ){
					$featured_text = __("Home",DOMAIN);
				}elseif( 'c' == $featured_type ){
					$featured_text = __("Category",DOMAIN);
				}elseif( 'both' == $featured_type ){
					$featured_text = __("Home, Category",DOMAIN);
				}
				//
				echo '<p>'.__('Package Name: ',DOMAIN). '<a href="'.site_url().'/wp-admin/admin.php?page=monetization&action=edit&package_id='.$package_id.'&tab=packages" >'.$package_name .'</a></p>';
				
				if($paid_amount >= 0){
					echo '<p>'.__('Total Price: ',DOMAIN).$paid_amount .'</p>';
				}
				if($featured_text !='-'){
					echo '<p>'.__('Featured: ',DOMAIN).$featured_text .'</p>';
				}
				if($expired_date){
					echo '<p>'.__('Exp Date: ',DOMAIN).$expired_date .'</p>';
					}
			}else{
				echo '-';	
			}
		...
		case 'email':						
			?>
							<tr class="post-submission">
								<td><label for="package_type" class="form-textfield-label"><?php _e('Admin email notification after post submission',DOMAIN); ?></label></td>
								<td>
									<a href="javascript:void(0);" onclick="open_quick_edit('post-submission','edit-post-submission')"><?php _e("Quick Edit",DOMAIN);?></a>
									| 
									<a href="javascript:void(0);" onclick="reset_to_default('post_submited_success_email_subject','post_submited_success_email_content','post-submission');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
								</td>
							</tr>
									...
									<table width="98%" class="tab-sub-table" align="left">
										<tr>
											<td style="line-height:10px">
												<label class="form-textfield-label sub-title"><?php _e('Subject',DOMAIN); ?></label>
											</td>
											<td width="90%" style="line-height:10px">
												<input type="text" name="post_submited_success_email_subject" id="post_submited_success_email_subject" value="<?php if(isset($tmpdata['post_submited_success_email_subject'])){echo stripslashes($tmpdata['post_submited_success_email_subject']);}else{_e('A new post has been submitted on your site',DOMAIN);} ?>"/>
											</td>
										</tr>
										<tr>
											<td style="line-height:10px">
												<label class="form-textfield-label sub-title"><?php _e('Message',DOMAIN); ?></label>
											</td>
											<td width="90%" style="line-height:10px">
												<?php
												$settings =   array(
											...
											<td colspan="2">
												<div class="buttons">
													<div class="inline_update">
														<a class="button-primary save  quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
							...
							<tr class="payment-success-client alternate">
								<td><label class="form-textfield-label"><?php _e('User email notification after successful payment',DOMAIN); ?></label></td>
								
								<td>
									<a href="javascript:void(0);" onclick="open_quick_edit('payment-success-client','edit-payment-success-client')"><?php _e("Quick Edit",DOMAIN);?></a> 
									| 
									<a href="javascript:void(0);" onclick="reset_to_default('payment_success_email_subject_to_client','payment_success_email_content_to_client','payment-success-client');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
								</td>
							</tr>
								...
								<td width="100%" colspan="2">
									...
									<table width="98%" align="left" class="tab-sub-table">
										<tr>
											...
											<td width="90%" style="line-height:10px">
												<input type="text" name="payment_success_email_subject_to_client" id="payment_success_email_subject_to_client" value="<?php if(isset($tmpdata['payment_success_email_subject_to_client'])){echo $tmpdata['payment_success_email_subject_to_client'];}else{_e('Thank you for your payment!',DOMAIN);} ?>"/>
											</td>
										</tr>
										<tr>
											<td style="line-height:10px">
										...
										<tr>
											<td colspan="2">
												<div class="buttons">
													...
													<a class="button-primary save alignleft quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
							...
							<tr class="payment-success-admin">
								<td><label class="form-textfield-label"><?php _e('Admin email notification after successful payment',DOMAIN); ?></label></td>
							
								<td>
									<a href="javascript:void(0);" onclick="open_quick_edit('payment-success-admin','edit-payment-success-admin')"><?php _e("Quick Edit",DOMAIN);?></a>
									| 
									<a href="javascript:void(0);" onclick="reset_to_default('payment_success_email_subject_to_admin','payment_success_email_content_to_admin','payment-success-admin');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
								</td>
							</tr>
							<tr class="edit-payment-success-admin" style="display:none">
								<td width="100%" colspan="2">
									<h4 class="edit-sub-title">Quick Edit</h4>
									<table width="98%" align="left" class="tab-sub-table">
										<tr>
											...
											<td width="90%" style="line-height:10px">
												<input type="text" name="payment_success_email_subject_to_admin" id="payment_success_email_subject_to_admin" value="<?php if(isset($tmpdata['payment_success_email_subject_to_admin'])){echo $tmpdata['payment_success_email_subject_to_admin'];}else{_e('You have received a payment',DOMAIN);} ?>"/>
											</td>
										</tr>
										<tr>
											<td style="line-height:10px">
							...
							<tr class="pre-payment-success-admin">
								<td><label class="form-textfield-label"><?php _e('Admin email notification after PreBank transfer',DOMAIN); ?></label></td>
							
								<td>
									<a href="javascript:void(0);" onclick="open_quick_edit('pre-payment-success-admin','edit-pre-payment-success-admin')"><?php _e("Quick Edit",DOMAIN);?></a>
									| 
									<a href="javascript:void(0);" onclick="reset_to_default('pre_payment_success_email_subject_to_admin','pre_payment_success_email_content_to_admin','pre-payment-success-admin');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
								</td>
							</tr>
										...
										<tr>
											...
											<td width="90%" style="line-height:10px">
												...
												if($tmpdata['pre_payment_success_email_content_to_admin'] != ""){
													...
													$content = __("<p>Howdy [#to_name#] ,</p><p>Payment from [#user_login#] is pending for the new listing they submitted on your site as they selected pre bank transfer as their preferred payment method.</p><p>You can view details below [#transaction_details#]</p> <p>You can contact [#user_login#] for status of the payment.</p><p>Thanks!<br/>[#site_name#]</p>",DOMAIN);
												}
										...
										<tr>
											<td colspan="2">
												<div class="buttons">
													<div class="inline_update">
														<a class="button-primary save  alignleft quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
														<a class="button-secondary cancel alignright" href="javascript:void(0);" onclick="open_quick_edit('edit-pre-payment-success-admin','pre-payment-success-admin')" accesskey="c">Cancel</a>
														<span class="save_error" style="display:none"></span><span class="spinner"></span>
														</div>
												</div>	
											</td>
					...
					<div class="tevo_sub_title" style="padding-top: 10px;"><?php _e('Notification Content Settings',DOMAIN); ?></div>
					<p class="tevolution_desc"><?php _e('These are the messages that appear on your site after certain actions (like content submissions).',DOMAIN)?></p>
					...
					<thead>
						<tr>
							...
							<th class="last-th">
								<label for="msg_desc" class="form-textfield-label"><?php _e('Actions',DOMAIN); ?></label>
							</th>
						</tr>
					</thead>
					<tbody>
						<tr class="post-submission-not alternate">
							<td><label class="form-textfield-label"><?php _e('Successful post submission message',DOMAIN); ?></label></td>
						
							<td><a href="javascript:void(0);" onclick="open_quick_edit('post-submission-not','edit-post-submission-not')"><?php _e("Quick Edit",DOMAIN);?></a> 
							| 
									<a href="javascript:void(0);" onclick="reset_to_default('','post_added_success_msg_content','post-submission-not');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
							</td>
						</tr>
									...
									<tr>
										...
										<td width="90%" style="line-height:10px">
											...
											if($tmpdata['post_added_success_msg_content'] != ""){
												...
												$content = '<p>'.__("Thank you! We have successfully received the submitted information.",DOMAIN).'</p><p><a href="[#submited_information_link#]">'.__("Click here",DOMAIN).'</a> '.__("to see the content you have just submitted.",DOMAIN).'</p><p>'.__("Thanks!",DOMAIN).'<br/> [#site_name#].</p>';
											}
									...
									<tr>
										<td colspan="2">
											<div class="buttons">
												<div class="inline_update">
													<a class="button-primary save alignleft  quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
													<a class="button-secondary alignright cancel " href="javascript:void(0);" onclick="open_quick_edit('edit-post-submission-not','post-submission-not')" accesskey="c"><?php _e('Cancel',DOMAIN);?></a>
													<span class="save_error" style="display:none"></span><span class="spinner"></span>
												</div>
											</div>	
										</td>
						...
						<tr class="payment-successful">
							<td><label class="form-textfield-label"><?php _e('Payment successfully received message',DOMAIN); ?></label></td>
							
							<td><a href="javascript:void(0);" onclick="open_quick_edit('payment-successful','edit-payment-successful')"><?php _e("Quick Edit",DOMAIN);?></a> 
							| 
									<a href="javascript:void(0);" onclick="reset_to_default('','post_payment_success_msg_content','payment-successful');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
							</td>
						</tr>
									...
									<tr>
										...
										<td width="90%" style="line-height:10px">
											...
											if($tmpdata['post_payment_success_msg_content'] != ""){
												...
												$content = '<h4>'.__("Your payment has been successfully received. The submitted content is now published.",DOMAIN).'</h4><p><a href="[#submited_information_link#]" >'.__("View your submitted information",DOMAIN).'</a></p><h5>'.__("Thank you for participating at",DOMAIN).' [#site_name#].</h5>';
											}
									...
									<tr>
										<td colspan="2">
											<div class="buttons">
												<div class="inline_update">
												<a class="button-primary save  quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
												<a class="button-secondary cancel " href="javascript:void(0);" onclick="open_quick_edit('edit-payment-successful','payment-successful')" accesskey="c">Cancel</a>
												<span class="save_error" style="display:none"></span><span class="spinner"></span>
												</div>
											</div>	
										</td>
						...
						<tr class="payment-cancel alternate">
							<td><label class="form-textfield-label"><?php _e('Payment cancelled message',DOMAIN); ?></label></td>
							
							<td><a href="javascript:void(0);" onclick="open_quick_edit('payment-cancel','edit-payment-cancel')"><?php _e("Quick Edit",DOMAIN);?></a> 
							| 
									<a href="javascript:void(0);" onclick="reset_to_default('','post_payment_cancel_msg_content','payment-cancel');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
							</td>
						</tr>
									...
									<tr>
										<td colspan="2">
											<div class="buttons">
												<div class="inline_update">
													<a class="button-primary save  quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
													<a class="button-secondary cancel" href="javascript:void(0);" onclick="open_quick_edit('edit-payment-cancel','payment-cancel')" accesskey="c">Cancel</a>
													<span class="save_error" style="display:none"></span><span class="spinner"></span>
												</div>
											</div>	
										</td>
						...
						<tr class="prebank-transfer">
							<td><label class="form-textfield-label"><?php _e('PreBank transfer success message',DOMAIN); ?></label></td>
							
							<td><a href="javascript:void(0);" onclick="open_quick_edit('prebank-transfer','edit-prebank-transfer')"><?php _e("Quick Edit",DOMAIN);?></a> 
							| 
									<a href="javascript:void(0);" onclick="reset_to_default('','post_pre_bank_trasfer_msg_content','prebank-transfer');"><?php _e("Reset",DOMAIN);?></a>
									<span class="spinner" style="margin:0 18px 0;"></span>
									<span class="qucik_reset"><?php _e("Data reset",DOMAIN);?></span>
							</td>
						</tr>
									...
									<tr>
										<td colspan="2">
											<div class="buttons">
												<div class="inline_update">
													<a class="button-primary save quick_save" href="javascript:void(0);" accesskey="s"><?php _e("Save Changes",DOMAIN);?></a>
													<a class="button-secondary cancel " href="javascript:void(0);" onclick="open_quick_edit('edit-prebank-transfer','prebank-transfer')" accesskey="c">Cancel</a>
