							  ...
							  { 
								if(get_post_meta($post->ID,"ctype",true) == 'multicheckbox')
								...
								else
								 {
									 ...
									 {
									 	echo "<li class='".$style_class."'><p class='tevolution_field_title label'>".$post->post_title." : </p> <p class='tevolution_field_title'> Click para descargar Imagen <a href=".get_post_meta($single_pos_id,$post->post_name,true).">Download</a></p></li>";
									 }
									 else
									 {
   		...
   		<?php do_action('templ_after_post_content'); /*Add Action for after the post content. */
			...
			?>
				<div class="row">
					<div class="title_space">
						<div class="title-container">
							<h1><?php _e('Mapa',DOMAIN); ?></h1>
						</div>
						<p><strong><?php _e('Localización : '); echo $add_str;?></strong></p>
					</div>
...
function send_friend_inquiry_email()
{
	...
	if (is_single() && ($post->post_type!='post' && $post->post_type!='page')) {
		...
		{			
			$claim_content_link='<a class="button small_btn tmpl_mail_friend" rel="leanModal_send_inquiry"  href="#inquiry_div" title="Send Inquiry" id="send_inquiry_id" >'.__('Send inquiry',DOMAIN).'</a>&nbsp;&nbsp;';	
			send_inquiry();			
			$link.= $claim_content_link;
		}
    ...
    <div class="row">
		 <?php if(count($post_images)>0): ?>
             <div class="content_details">
                 <div class="graybox">
                        <img width="400px;" height="400px;" id="replaceimg" src="<?php echo $post_images;?>" alt="<?php echo $img_alt; ?>" title="<?php echo $img_title; ?>" />
                 </div>
             </div>            
         <?php endif; ?>
...
function related_post_by_categories()
{
	...
	if(is_plugin_active('wpml-translation-management/plugin.php') && (!empty($tmpdata['related_post_type']) && in_array($post->post_type,$tmpdata['related_post_type']))){
	...
	if( $my_query->have_posts() ) :
	 ?>
     <!-- MATTHIAS -->
     <!-- Agregué el display none -->
     <div style="display:none;" class="realated_post clearfix">  
     	<!-- MATTHIAS --> 
    	 <h3><span>Más Listados <?php //echo __("Related",DOMAIN).' '.$post_lable;?></span></h3>
...
function category_post_categories_tags()
{
	...
	if(!empty($terms) && (!empty($htmlvar_name['[#taxonomy_name#]']['category']) || !empty($htmlvar_name['category'])))
	{
	...
	if(!empty($tag_terms) && (!empty($htmlvar_name['[#taxonomy_name#]']['category']) || !empty($htmlvar_name['category'])))
	{
		echo '<p class="bottom_line"><span class="i_category">';
...
function single_post_categories_tags()
{
	...
	echo ($taxonomy_category!='')?sprintf(__('Tagged In %s',DOMAIN),$taxonomy_category):'';
}
...
function post_info()
{
    ...
    <div class="byline">
		...
		if ( !current_user_can( $post_type->cap->edit_post, get_the_ID() ) ){
			...
			$edit = '<span class="post_edit"><a class="post-edit-link" href="' . esc_url( get_edit_post_link( get_the_ID() ) ) . '" title="' . sprintf( esc_attr__( 'Edit %1$s', DOMAIN ), $post_type->labels->singular_name ) . '">' . __( 'Edit', DOMAIN ) . '</a></span>';
		}	
		$author = __('Published by',DOMAIN).' <span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( get_the_author_meta( 'display_name' ) ) . '">' . get_the_author_meta( 'display_name' ) . '</a></span>';
		$published = __('On',DOMAIN).' <abbr class="published" title="' . sprintf( get_the_time( esc_attr__( get_option('date_format')) ) ) . '">' . sprintf( get_the_time( esc_attr__( get_option('date_format')) ) ) . '</abbr>';
	    echo sprintf(__('%s %s %s %s',DOMAIN),$author,$published,$write_comments,$edit);
        ?>
    </div>
    <?php		
}
...
function templ_taxonomy_category_content()
{
	...
	if($tmpdata['listing_hide_excerpt']=='' || !in_array(get_post_type(),$tmpdata['listing_hide_excerpt'])){
		...
		if($theme_settings['supreme_archive_display_excerpt'] && (!empty($htmlvar_name['[#taxonomy_name#]']['post_excerpt']) || !empty($htmlvar_name['post_excerpt'])) ){
			...
			echo '</div>';
		}elseif(!empty($htmlvar_name['[#taxonomy_name#]']['post_content']) || !empty($htmlvar_name['post_content'])){
...
function home_page_feature_listing_orderby($orderby)
{
	global $wpdb,$wp_query;
	if(is_home() || @is_front_page()){		
		$orderby = " (SELECT DISTINCT($wpdb->postmeta.meta_value) from $wpdb->postmeta, $wpdb->posts where ($wpdb->posts.ID = $wpdb->postmeta.post_id) AND $wpdb->postmeta.meta_key = 'featured_type' AND $wpdb->postmeta.meta_value = 'h') DESC,$wpdb->posts.post_date DESC";
	}
	return $orderby;
}
