...
<div id="templatic_posttype" class="widget_div">
	...
	<div class="inside">
       ...
       <a href="<?php echo site_url(); ?>/wp-admin/admin.php?page=custom_taxonomy"> <h3 class="hndle"><span><?php _e('Custom Post Types Manager',DOMAIN); ?></span></h3></a>
		<p class="mod_desc"><?php
		_e('Creating a new custom post type is tricky if you&lsquo;re not very familiar with PHP & WordPress coding - This module solves that problem. With this manager you can easily create new post types and taxonomies. Every created post type / taxonomy will work flawlessly with custom fields and price packages.',DOMAIN);
		?></p>
        </div>
		<?php if(!is_active_addons('custom_taxonomy')) { ?>
		<div id="publishing-action" class="settings_style">
		<a id="publishing_action_custom_taxonomy" href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&activated=custom_taxonomy&true=1";?>" class="button-primary"><i class="fa fa-check"></i>&nbsp;<?php _e('Activate &rarr;',DOMAIN); ?></a></div>
		<?php } 
		if (is_active_addons('custom_taxonomy')) : ?>
		<div id="publishing-action" class="settings_style">
			<a title="<?php _e('Settings',DOMAIN); ?>" id="custom_taxonomy_setting" class="templatic-tooltip set_lnk" href="<?php echo site_url()."/wp-admin/admin.php?page=custom_taxonomy";?>"><i class="fa fa-gear"></i></a> 
			<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&deactivate=custom_taxonomy&true=0";?>" class="button"><i class="fa fa-times"></i>&nbsp;<?php _e('Deactivate ',DOMAIN); ?></a> 
