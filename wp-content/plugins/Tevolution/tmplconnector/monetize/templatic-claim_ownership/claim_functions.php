...
function fetch_claims()
{
	...
	if(count($claim_post_ids) != 0)
	{
		...
		foreach ($claim_post_ids as $claim_post_id) :			
			...
			?>
               <tr>
				<td><?php echo $claim_post_id;?></td>
                    <td>
                    	<a href="<?php echo site_url().'/wp-admin/post.php?post='.$post_id.'&action=edit';?>" title="<?php echo VIEW_CLAIM; ?>"><?php echo $post_title?></a>
                    </td>
                    <td><?php echo $udata->display_name;?></td>
                    <td><?php echo $claimer_name;?></td>
                    <td><?php echo $claimer_contact;?></td>                    
		...
		else
		{
			...
			foreach($post_claim_id as $key => $val) :
				...
				if($data['claim_status'] == 'pending') :
                    ...
                    <?php endif;// Finish claim status pending if condition 
...
function insert_claim_ownership_data($post_details)
{
	...
	if(@$_POST['claimer_name'])
	{
		...
		$claim =  __('<p>Dear admin,</p><br/>
			...
			<p>[#message#]</p>
			<p>Link :[#post_title#]</p>
			<p>From : [#your_name#]</p><p>Phone Number : [#your_number#]</p>',DOMAIN);
	...
	if(@$_REQUEST['claimer_name'])
	{
		...
		if( $tmpdata['recaptcha'] == 'recaptcha')
		{
			if(file_exists(get_tmpl_plugin_directory().'wp-recaptcha/recaptchalib.php') && is_plugin_active('wp-recaptcha/wp-recaptcha.php') && in_array('claim',$display))
			{
				...
				if ($resp->is_valid )
				{
					echo __('Your claim for this post has been sent successfully.',DOMAIN);
					exit;
				}
				else
				{
