...
if(isset($_REQUEST['activated']) && $_REQUEST['activated']=='claim_ownership'){
	templatic_module_activationmsg('claim_ownership','Claim Ownership','',$mod_message=__('Now move on to the',DOMAIN).' <a href='.admin_url('/admin.php?page=templatic_settings').'>'.__('General Settings',DOMAIN).'</a> '.__('page to select the post types for your site. You need to set up the widget in order to show the',DOMAIN).' <strong>'.__('Claim for this post',DOMAIN).'</strong> '.__('link. You can set up the widget',DOMAIN).' <a href='.admin_url('/widgets.php').'>'.__('here',DOMAIN).'</a>.',$realted_mod =''); 
...
<div id="templatic_claimownership" class="widget_div">
	...
	<div class="inside">
		<div class="t_module_desc">
         <a href="<?php echo site_url(); ?>/wp-admin/admin.php?page=templatic_settings#general_claim_setting"><h3 class="hndle"><span><?php _e('Claim Post Manager',DOMAIN); ?></span></h3></a>
        <p class="mod_desc"><?php
		_e('Claim Post functionality will allow your site visitors to claim posts from you or other members. Once a post is claimed you will have to verify it and make the necessary changes. This functionality will/can work for all your created post types. ',DOMAIN);
		?></p>
        </div>
		<?php if(!is_active_addons('claim_ownership')) { delete_option('claim_enabled'); ?>
		<div id="publishing-action" class="settings_style">
		<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&activated=claim_ownership&true=1";?>" class="button-primary"><i class="fa fa-check"></i><?php _e('Activate &rarr;',DOMAIN); ?></a></div>
		<?php } 
		...
		<div id="publishing-action" class="settings_style">
			...
			<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&deactivate=claim_ownership&true=0";?>" class="button"><i class="fa fa-times"></i>&nbsp;<?php _e('Deactivate',DOMAIN); ?></a>
