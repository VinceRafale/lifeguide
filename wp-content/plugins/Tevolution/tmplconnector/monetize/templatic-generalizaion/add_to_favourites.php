...
function remove_from_favorite($post_id,$language='')
{
	...
	update_user_meta($current_user->ID, 'user_favourite_post', $user_meta_data); 	
	echo '<a class="addtofav" href="javascript:void(0);"  onclick="javascript:addToFavourite(\''.$post_id.'\',\'add\');">'.__(ADD_FAVOURITE_TEXT,DOMAIN).'</a>';
}
/*
...
function tevolution_favourite_html($user_id='',$post='')
{
	...
	if($post->post_type !='post'){
		if(is_tax()){ $class=""; }else{ if(!isset($_GET['post_type']) && @$_GET['post_type']==''){ $class="button"; }else{ $class="";  } }  
