	...
	function fetch_package_feature_details_backend($edit_id='',$png_id='',$all_cat_id){	
		...
		?>
			...
			<div class="form_row clearfix is_backend_featured" id="is_featured" <?php echo ($is_featured!=1) ?'style="display:none"':'';?>>
				<label><strong><?php echo sprintf(__('Would you like to make this %s featured?',DOMAIN) ,$post_type);?></strong></label>
			...
			<!-- DISPLAY THE PACKAGE IN FRONT END -->	
				<div class="package">
				 ...
				 <?php   
						...
						{
							echo ($package_validity==1)? __('Day',DOMAIN):__(DAYS_TEXT,DOMAIN);
						}
						elseif($package_validity_per == 'M')
						{
							echo ($package_validity==1)? __('Month',DOMAIN):__(MONTHS_TEXT,DOMAIN);
						}
						else
						{
							echo ($package_validity==1)?__('Year',DOMAIN) :__(YEAR_TEXT,DOMAIN);
						}
					 ...
					 if($package_is_recurring=='1')
					 {
						...
						if($package_billing_per == 'D')
						{
							echo ($package_billing_num==1)? __('Day',DOMAIN):__(DAYS_TEXT,DOMAIN);
						}
						elseif($package_billing_per == 'M')
						{
							echo ($package_billing_num==1)? __('Month',DOMAIN):__(MONTHS_TEXT,DOMAIN);
						}
						else
						{
							echo ($package_billing_num==1)?__('Year',DOMAIN) :__(YEAR_TEXT,DOMAIN);
						}
						echo "&nbsp;".$package_billing_cycle."&nbsp;Cycle. </span>";
					 }
		...
		if(isset($edit_id) && $edit_id !=''){
			...
			<div class="form_row clearfix" id="is_featured" <?php if($is_featured == '1'){ }else{ ?>style="display:none; } <?php }?>">
				...
				<div class="feature_label">
					...
					<label><input type="checkbox" name="featured_h" id="featured_h" value="<?php echo $featured_h; ?>" onclick="featured_list(this.id)" <?php if(@$_SESSION['custom_fields']['featured_h'] !=""){ echo "checked=checked"; } ?>/><?php _e(FEATURED_H,DOMAIN); ?> <span id="ftrhome"><?php if(isset($featured_h) && $featured_h !=""){ echo "(".fetch_currency_with_position($featured_h).")"; }else{ echo "(". __('Free',T_DOMAIN) .")"; } ?></span></label>
			...
			<label><?php echo sprintf(__('Would you like to make this %s comment be moderate?',DOMAIN) ,$post_type);?> </label>
			<?php
						if(is_plugin_active('thoughtful-comments/fv-thoughtful-comments.php')){
			...
			<div id="price_package_price_list" <?php if($_SESSION['custom_fields']['total_price'] <= 0  || $_SESSION['custom_fields']['total_price'] == ''){?> style="display:none;" <?php } ?> class="form_row clearfix">
				<label><?php _e(TOTAL_TEXT,DOMAIN);?> <span>*</span> <?php echo '<span class="message_note">';_e("Final amount including discount will be displayed on preview page.",DOMAIN);echo "</span>";?></label>
				<div class="form_cat"  >
				<?php if(isset($category_id)){ $catid = $category_id; }else{ $catid =''; }
					...
					<span id="cat_price_total_price" <?php if(($fprice <= 0 || $fprice == '') && ($cat_price <= 0 || $cat_price == '') || ($fprice <= 0 || $fprice == '') && ($packprice <= 0 || $packprice == '') || ($packprice <= 0 || $packprice == '') && ($cat_price <= 0 || $cat_price == '')){?> style="display:none;" <?php } ?>><?php echo "="; ?>
					...
					<?php if($position == '3'){ echo $currency; }else if($position != 1 && $position != 2 && $position !=3){ echo '&nbsp;'.$currency; } ?></span>
...
<?php
	...
	if($recent_posts){
		...
		$th='	<tr>
				...
				<th valign="top" align="left">'.__('Type',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Pay Method',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Package',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Amount',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Featured',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Exp Date',DOMAIN).'</th>
				<th valign="top" align="left">'.__('Date',DOMAIN).'</th>
		...
		foreach($recent_posts as $posts) {			
			...
			echo '<tr>
				<td valign="top" align="left"><a href="'.admin_url().'admin.php?page=transcation&action=edit&trans_id='.$posts->ID.'">'.$posts->ID.'</a></td>
			...
			echo '<td valign="top" align="left">'.get_the_title($tran_info[0]->package_id).'</td>';
			echo '<td valign="top" align="left">'.$price_amount.'</td>';
			echo '<td valign="top" align="left">'.$featured_text.'</td>';
			echo '<td valign="top" align="left">'.$expired_date.'</td>';
			echo '<td valign="top" align="left">'. date_i18n(get_option("date_format"),strtotime($tran_info[0]->payment_date)).'</td>';
				if($no_alive_days !='1')
				{
		...
		echo '<p style="margin:0 0 10px">'.sprintf(__('No transaction type selected from  <a href="%s" >transaction settings</a>.',DOMAIN),admin_url( 'admin.php?page=transcation' )).'</p>';
	}
}
...
function price_package_meta_box(){
	...
	?>
     <p><label><?php _e('Package Name: ',DOMAIN);?></label><strong><?php echo $package_name;?></strong></p>
     <p><label><?php _e('Total Amount: ',DOMAIN);?></label><strong><?php echo $paid_amount;?></strong></p>
     <p><label><?php _e('Alive Days: ',DOMAIN);?></label><strong><?php echo $alive_days;?></strong></p>
     <p><label><?php _e('Featured for home page? : ',DOMAIN);?></label><strong><?php echo $featured_h;?></strong></p>
     <p><label><?php _e('Featured for category page? : ',DOMAIN);?></label><strong><?php echo $featured_c;?></strong></p>
    <?php
}
/*
