...
<p class="tevolution_desc"> <?php _e('Whatever sales are done on your site are recorded and displayed here as the transactions. Few things that you can perform here are easily changing the payment status manually (if you want), search for particular transaction using the below given fields, sort your all transactions according to your payment gateway by clicking the column "Payment Method" and last but not the least, you can also export all your transactions to CSV file. <a href="javascript:void(0);" title="Plugins" id="trans_color_setting">You can change transcation color settings from here.</a>.',DOMAIN);?></p>
	<div class="tevolution_normal ordersearch">
		<div id="poststuff">
		<div class="postbox">
				<h3 class="hndle"><span><?php _e('Generate Transaction Report',DOMAIN)?></span></h3>
    ...
    <form method="post" action="" name="ordersearch_frm">
        <table class="form-table" cellspacing="1" cellpadding="4" border="0" >
			...
			<tr>
				<th valign="center"><?php _e('Search by transaction ID',DOMAIN); ?></th>
				<td valign="center"><input type="text" class="regular-text" value="" name="srch_orderno" id="srch_orderno" />&nbsp;<input type="submit" name="Search" value="<?php _e('Search',DOMAIN); ?>" class="button-primary"  />
                    <p class="description"><?php _e('Enter unique Id to search transaction', DOMAIN);?></p></td>
            </tr>
            <tr style="border-top: 1px solid #ccc; ">
				<th  valign="center"><?php _e('Post Type',DOMAIN); ?></th>
				<td valign="center">	
				<?php
            ...
            <?php
				...
				$i++;	
        ?></select><br /><p class="description"><?php _e('Select the post type, transactions of which you want to search', DOMAIN);?></p></td>
        	</tr>
            <tr>
				<th valign="center"><?php _e('Payment Type',DOMAIN); ?></th>
				<td valign="center">
				<?php
					...
					</select><p class="description"><?php _e('Select the payment method (gateway), using which transactions have been done.', DOMAIN);?></p></td>
            </tr>
			<tr>	
				<th valign="center"><?php _e('Name/Email',DOMAIN); ?></th>
				<td valign="center" colspan="4"><input type="text" class="regular-text" value="" name="srch_name" id="srch_name" /><br /><p class="description"><?php _e('Enter the name or email Id using which transactions have been done', DOMAIN);?></p></td>
			</tr>
			<tr>
				<?php
				...
				<?php } 
				?>
				<th valign="center"><?php _e('Search by transaction date',DOMAIN); ?></th>
				<td valign="center" ><input type="text" PLACEHOLDER="<?php _e('From',DOMAIN); ?>" class="regular-text" value="<?php if(isset($_REQUEST['trans_from_date']) && $_REQUEST['trans_from_date']!= ''){ echo $_REQUEST['trans_from_date'];}?>" name="trans_from_date" id="trans_from_date" />
				<input type="text" PLACEHOLDER="<?php _e('To',DOMAIN); ?>" class="regular-text" value="<?php if(isset($_REQUEST['trans_to_date']) && $_REQUEST['trans_to_date']!= ''){ echo $_REQUEST['trans_to_date'];}?>" name="trans_to_date" id="trans_to_date" /><p class="description"><?php _e('Select the time duration in which transactions have been done.', DOMAIN);?></p></td>
			</tr>
			<?php	do_action('add_fields_after_transaction_fields');		?>
			<tr>
				<th></th>
				<td valign="center"><input type="submit" name="Search" value="<?php _e('Search',DOMAIN); ?>" class="button-primary"  />&nbsp;<input type="submit" name="Reset" value="<?php _e('Reset',DOMAIN); ?>"  class="button-secondary action" /></td>
        	</tr>
			
            <tr style="border-top: 1px solid #ccc; "><br/>
            	<td colspan="2"><p><?php _e('Export the transaction data from here ',DOMAIN); ?>&nbsp;&nbsp;<a class="button-primary" href="<?php echo plugin_dir_url( __FILE__ ).'export_transaction.php';?>" title="Export To CSV" class="i_export"><?php _e('Export To CSV',DOMAIN);?></a></p></td>
            </tr>
    </table>
	</form>
	</div>
	...
	<div style="display:none;" id="trans_frm_id" class="tevolution_normal ordersearch" class="tevolution_normal ordersearch">
		<div id="poststuff">
			<div class="postbox">
				<h3 class="hndle"><span><?php _e('Transaction color settings',DOMAIN); ?></span></h3>
	<div class="transaction_page_set">
	<form  method="post" action=""  name="transaction_frm">
		...
		?>
		<p class="tevolution_desc"><?php _e('You can select the different colors for the different post types here which will be applied to that transaction report.It basically helps you in differentiating the transactions done with various post types.',DOMAIN); ?></p>
		<table class="form-table tbl_transaction_frm">
			<tr>
				<th valign="center"><label><?php _e('Transaction Settings',DOMAIN);?></label></th>
				<td valign="center">
				   <div class="element">
		...
		<div class="clearfix"></div>
			 ...
			 <tr>
				<td></td>
				<td><p style="clear: both;" class="submit"><input type="submit" value="<?php _e('Save All Settings',DOMAIN);?>" class="button-primary" name="trans_setting"></td>
			 </tr>
		</table>	
	</form>
	</div>
