...
{  	
	if (str=="")
	  ...
	  url = "<?php echo plugin_dir_url( __FILE__ ); ?>ajax_categories_dropdown.php?post_type="+valarr+'&page=monetization'
...
<div class="wrap">	
	...
	<tbody>
		<tr class="" id="package_type">
			...
			<td>
				...
				<span id="pay_per_post_desc" class="description" <?php echo $show_desc; ?> ><?php echo PER_POST_DESC; ?></span>
				<span id="pay_per_sub_desc" class="description" <?php echo $show_desc2; ?>><?php echo PER_SUBSCRIPTION_DESC; ?></span>
			</td>
		</tr>
		...
		<tr>
			...
			?>
               	...
               	<fieldset>				
				<label for="selectall_post_type"><input type="checkbox" name="package_post_type[]" id="selectall_post_type" onClick="showcategory(this.value);selectall_price_package_posttype();" value="all,all"  <?php if(in_array('all',$pkg_post_type)){ echo 'checked="checked"';}?>/>&nbsp;<?php _e('Select All', DOMAIN);?></label><br />
				
                    <label for="post_type_post"><input type="checkbox" name="package_post_type[]" id="post_type_post" onClick="showcategory(this.value);" value="post,category" <?php if(in_array('post',$pkg_post_type) || in_array('all',$pkg_post_type)){ echo 'checked="checked"';}?> />&nbsp;<?php echo 'Post';?></label><br />
				<?php
				$i=1;			
				foreach ($post_types as $key => $post_type) {	
					...
					<label for="post_type_<?php echo $i; ?>"><input type="checkbox" name="package_post_type[]" id="post_type_<?php echo $i; ?>" onClick="showcategory(this.value);" value="<?php echo $key.",".$slugs; ?>" <?php if(in_array($key,$pkg_post_type) || in_array('all',$pkg_post_type)){ echo 'checked="checked"';}?>/>
		...
		<tr>
			...
			<td>
				<div class="element wp-tab-panel" id="field_category">
				<?php if(@$_REQUEST['tab'] =='packages' && $_REQUEST['action'] =='edit'){ ?>
				<li>	<label for="selectall_post_type clearfix"><input type="checkbox" onclick="displaychk_frm();" class="checkbox" id="selectall" name="selectall">&nbsp;<?php _e('Select All', DOMAIN);?></label></li>
				<?php }
				$pctype = '';
					if(isset($id) && $id != '')
					{
						...
						{
							get_wp_category_checklist_plugin('category',$pid);
							foreach ($post_types as $key => $post_type)
							{
		...
		<tr class="">
			...
			<td>
				...
				&nbsp;<label for="package_status"><?php _e("Yes",DOMAIN); ?></label><br/>
                <p class="description"><?php _e('Check marking it will activate this package for the post categories selected above on your site.',DOMAIN);?></p>
			</td>
		</tr>
		<tr>
			<th valign="top">
			...
			<td>
				...
				<br/>
				<span class="description"><?php echo RECURRING_DESC;?><b><?php _e("Works only with PayPal!",DOMAIN);?></b></span>
			</td>
		</tr>
	...
	<tbody>
		<tr>
			...
			<td>
				...
				<span class="description"><?php echo THOUGHTFUL_COMMENT_STATUS_DESC; ?>.</span>
			</td>
		</tr>
	...
	<?php } ?>
		<tr>
			<td colspan="2"><input type="submit" class="button-primary form-submit form-submit submit" value="<?php _e('Save Settings',DOMAIN); ?>" name="submit" id="submit-1"></td>
		</tr>
	</tbody>
	</table>
	</form>
