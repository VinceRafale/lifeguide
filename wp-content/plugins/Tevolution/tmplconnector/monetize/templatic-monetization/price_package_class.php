...
class templatic_List_Table extends WP_List_Table 
{
	...
	* Call a function that will return all the data in an array and we will assign that result to a variable $package_data. FIRST OF ALL WE WILL FETCH DATA FROM POST META TABLE STORE THEM IN AN ARRAY $package_data */
	function fetch_package_meta_data( $post_id = '' ,$post_title = '')
	{
		...
		if( $validity_per == 'D' )
		{
			$validity_per = $validity."&nbsp;".__('Days',DOMAIN);
		}
		elseif( $validity_per == 'M' )
		{
			$validity_per = $validity."&nbsp;".__('Months',DOMAIN);
		}
		else
		{
			$validity_per = $validity."&nbsp;".__('Years',DOMAIN);
		}
		if( $status == '1' )
		{
			$package_status = "<font color='green'>".__('Active',DOMAIN)."</font>";
		}
		else
		{
			$package_status = "<font color='red'>".__('Inactive',DOMAIN)."</font>";
		}
		if( $featured == '1' )
		{
			$ftype = "Yes";
		...
		if($package_type ==2){
			$package_type = __('Pay per subscription',DOMAIN);
		}else{
			$package_type = __('Pay per post',DOMAIN);
		}
		$meta_data = array(
						'ID'				=> $post_id,
	...
	function get_columns()
	{
		$columns = array(
					...
					'package_type'   => __('Type',DOMAIN),
					'package_amount' => PACKAGE_AMOUNT,			
					'validity'       => BILLING_PERIOD,
					'is_featured'    => IS_FEATURED.'?',
					'package_status' => __('Status',DOMAIN),
				);
		return $columns;
	}
