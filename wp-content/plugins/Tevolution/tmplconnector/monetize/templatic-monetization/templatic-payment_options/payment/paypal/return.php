...
<?php 
	...
	if($_REQUEST['pid']!="" && $_REQUEST['trans_id']!=""){
		global $wpdb,$transection_db_table_name;
		$wpdb->query("UPDATE $transection_db_table_name SET status=1 and paypal_transection_id ='".$_REQUEST['txn_id']."' where trans_id = '".$_REQUEST['trans_id']."'");
	...
	$transaction_details="";
	$transaction_details .= "--------------------------------------------------\r\n";
	$transaction_details .= "Payment Details for $post_name\r\n";
	$transaction_details .= "--------------------------------------------------\r\n";
	if($txn_id)
		$transaction_details .= "	Paypal Trans ID: $txn_id\r\n";
	if($trans_id)
		$transaction_details .= "   Transaction ID: $tarns_id\r\n";
	if($payment_status !='')
		$transaction_details .= "	Status: $payment_status\r\n";
	if($payment_type !='')
		$transaction_details .= "   Type: $payment_type\r\n";
	$transaction_details .= "   Date: $payment_date\r\n";
	$transaction_details .= "   Method: $txn_type\r\n";
	$transaction_details .= "--------------------------------------------------\r\n";
	...
	$transaction_details ="";
	$transaction_details .= "--------------------------------------------------\r\n";
	$transaction_details .= "Payment Details for $post_title\r\n";
	$transaction_details .= "--------------------------------------------------\r\n";
		if($txn_id)
		$transaction_details .= "	Paypal Trans ID: $txn_id\r\n";
	if($trans_id)
