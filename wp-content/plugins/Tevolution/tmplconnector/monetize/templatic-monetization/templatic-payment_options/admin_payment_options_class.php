...
class wp_list_payment_options extends WP_List_Table 
{
	...
	* Call a function that will return all the data in an array and we will assign that result to a variable $payment_options. FIRST OF ALL WE WILL FETCH DATA FROM POST META TABLE STORE THEM IN AN ARRAY $payment_options */
	function templ_get_pay_option_data($payment_method = '')
	{
		...
		//echo "<pre>"; print_r($paymentInfo->option_value); echo "</pre>";
		...
		if((isset($paymentInfo->isactive) && $paymentInfo->isactive !='') || (isset($paymentInfo->autoload) && $paymentInfo->autoload == 'yes')){ $status = "<span style='color:green; font-weight:normal;'>".__("Activated",DOMAIN)."</span>"; }else{ $status = "<span style='color:red; font-weight:normal;'>".__("Deactivated",DOMAIN)."</span>"; } /* display satus */
