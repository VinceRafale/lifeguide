               	...
               	<h5 class="payment_head"> 
					<?php 
						...
						if(function_exists('icl_register_string')){
							icl_register_string(DOMAIN,$pay_with_title,$pay_with_title);
						}
						
						if(function_exists('icl_t')){
							$pay_with_title1 = icl_t(DOMAIN,$pay_with_title,$pay_with_title);
						}else{
							$pay_with_title1 = __($pay_with_title,DOMAIN); 
						}
               ...
               <?php else:?>
				<h5 class="payment_head"> 
					<?php 
						...
						if(function_exists('icl_register_string')){
							icl_register_string(DOMAIN,$select_payment_method_title,$select_payment_method_title);
						}
						
						if(function_exists('icl_t')){
							$select_payment_method_title1 = icl_t(DOMAIN,$select_payment_method_title,$select_payment_method_title);
						}else{
							$select_payment_method_title1 = __($select_payment_method_title,DOMAIN); 
						}
		...
		<li id="<?php echo $paymentInfo['key'];?>">
		  <label><input <?php echo $jsfunction;?>  type="radio" value="<?php echo $paymentInfo['key'];?>" id="<?php echo $paymentInfo['key'];?>_id" name="paymentmethod" <?php echo $chked; if($disable_input){echo "disabled=true";}?> />  
						<?php 
							...
							{
								$payment_display_name = sprintf(__('%1$s',DOMAIN),$payment_display_name);
							}
...
function templ_payment_methods(){ 
	...
	?>
<div class="wrap">
	<div class="tevo_sub_title"><?php _e('Manage Currency',DOMAIN); ?></div>
	<p class="tevolution_desc"><?php _e('Define the currency in which you want to take payment on your site, you can set currency position with its amount as per your currency standards.',DOMAIN); ?></p>
	<?php
			...
			<tbody>
				<tr >
					...
					<div id="cur_sym" class="currency_sets">
						<input type="text" class="" class="form-radio radio" value="<?php echo get_option('currency_symbol'); ?>" name="currency_symbol" id="currency_symbol" PLACEHOLDER="<?php _e('Currency Symbol',DOMAIN); ?>"/>
                       
					</div>
					<div id="cur_code" class="currency_sets">
						<input type="text" class="" class="form-radio radio" value="<?php echo get_option('currency_code'); ?>" name="currency_code" id="currency_code" PLACEHOLDER="<?php _e('Currency Code',DOMAIN); ?>"/>
						
					</div>
					<p class="description"><?php _e('Your currency symbol can be any character like alphabets, numbers, alplhanumeric etc',DOMAIN);?> <span class="description"><?php echo  CURRENCY_CODE_DESC; ?></p>
					</td>
				</tr>
				...
				<tr>
					<td colspan="2">
						<input type="submit" class="button-primary form-submit form-submit submit" value="<?php _e('Save Currency',DOMAIN);?>" name="submit_currency" id="submit_currency">
					</td>
				</tr>
			</tbody>
			</table>
		...
		<div class="tevo_sub_title"><?php _e('Manage Payment Options',DOMAIN); ?></div>
		<p class="tevolution_desc"><?php _e('Manage the available payment gateways. To download and install more please visit the <a href="http://templatic.com/members/member" title="Plugins" target="_blank">member area (Plugins Download section)</a>',DOMAIN); ?>.</p>
 ...
 * Return: sort order of payment gatway
 */
