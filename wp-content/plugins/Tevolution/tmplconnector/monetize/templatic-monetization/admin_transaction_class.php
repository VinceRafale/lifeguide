	...
	* Call a function that will return all the data in an array and we will assign that result to a variable $transaction_data. FIRST OF ALL WE WILL FETCH DATA FROM TRANSACTION TABLE STORE THEM IN AN ARRAY $transaction_data */
	...
	function fetch_transction()
	{
		...
		{ 
			 foreach($transinfo as $transinfoObj) :
				...
				if( 'h' == $featured_type ){
					$featured_text = '<div>'.__("Featured",DOMAIN).': '.__("Home",DOMAIN).'</div>';
				}elseif( 'c' == $featured_type ){
					$featured_text = '<div>'.__("Featured",DOMAIN).': '.__("Category",DOMAIN).'</div>';
				}elseif( 'both' == $featured_type ){
					$featured_text = '<div>'.__("Featured",DOMAIN).': '.__("Home, Category",DOMAIN).'</div>';
				}else{
					$featured_text = '';
				}
				...
				$is_recurring = ( @get_post_meta($post_package->ID,'recurring',true)) ? '<div>'.__("Recurring",DOMAIN).'</div>' : '';				
				...
				if($transinfoObj->trans_id !=''){
					$trans_id = '<div>'.__('ID:',DOMAIN).' <a href="'.site_url().'/wp-admin/admin.php?page=transcation&action=edit&trans_id='.$transinfoObj->trans_id.'">'.$transinfoObj->trans_id.'</a></div>';
				}else{
					$trans_id ='';
				}
				$transaction_data[] =  array(
										...
										'title'		  => '<a href="'.site_url().'/wp-admin/user-edit.php?user_id='.$transinfoObj->user_id.'">'.$transinfoObj->billing_name.'</a><div>'.__('Email:',DOMAIN).' '.$transinfoObj->pay_email.'</div>',
	...
	function get_columns()
	{
		$columns = array(
					...
					'post_type'     => __('Posted In',DOMAIN),
					'payment_method'=> __('Payment Method',DOMAIN),
					'package'       => __('Price Package',DOMAIN),
					'amount'        => __('Amount',DOMAIN),
					'exp_date'      => __('Exp. Date',DOMAIN),
					'tran_date'     => __('Date',DOMAIN),
