...
class Templatic_connector { 
	...
	*/
	...
	function templ_extend(){
		...
		require_once(TEMPL_MONETIZE_FOLDER_PATH.'templ_header_section.php' );
		?>
          <p class="tevolution_desc"><?php _e('The plugins listed below will help you completely transform your website. For instance, installing the Booking plugin will allow you to manage a smaller hotel or a rental home. ',DOMAIN);?></p>
          <?php
		echo '<div><div>
		<div id="tevolution_bundled" class="metabox-holder wrapper widgets-holder-wrap"><table cellspacing="0" class="wp-list-tev-table postbox fixed pages ">
			<thead>
	...
	function templ_payment_gateway(){
		...
		require_once(TEMPL_MONETIZE_FOLDER_PATH.'templ_header_section.php' );
		?>
          <p class="tevolution_desc"><?php _e('Offer new ways to pay using the plugins available below. You will also need a merchant account (with the payment processor) in order to use the downloaded plugin.',DOMAIN);?></p>
          <?php
