...
function tevolution_setup_steps_box_display(){ 
	...
	$setup_steps = array(
				 array('title'=>__('Create a Post type',DOMAIN),
				'link'=> site_url().'/wp-admin/admin.php?page=custom_taxonomy',
				'desc'=>sprintf(__('Create your own <a href="%s/wp-admin/admin.php?page=custom_taxonomy">post type</a> (e.g. events, places etc) and extend default WordPress functionality to manage your content.',DOMAIN),site_url()),
				'status'=> $setup_post_type),
				 array('title'=>__('Set the Permalink',DOMAIN),
				'link'=> site_url().'/wp-admin/options-permalink.php',
				'desc'=>sprintf(__('Set the  <a href="%s/wp-admin/options-permalink.php">Permalink</a> after creating the custom post type so wordpress can easily flush the permalink rule for your new created post type.',DOMAIN),site_url()),
				'status'=> $permalink_status),
				array('title'=>__('Setup the Page settings',DOMAIN),
				'link'=> site_url()."/wp-admin/admin.php?page=templatic_settings",
				'desc'=>sprintf(__('Go to <a href="%s/wp-admin/admin.php?page=templatic_settings">General settings</a> to setup the settings for all your pages which includes how your listing page, detail page and submission form are going to work, also what specific things you want to include on each of the pages mentioned here.',DOMAIN),site_url()),
				'status'=> $other_flag), 
				 array('title'=>__('Create a Submission Form',DOMAIN),
				'link'=> site_url()."/wp-admin/post-new.php?post_type=page",
				'desc'=>sprintf(__('Add a new <a href="%s/wp-admin/post-new.php?post_type=page">WordPress page</a> and add this shortcode [submit_form post_type=\'your post type\'] in its editor to create a submission form for the post type you have created. Your site visitors will be able to post their entries(e.g. Events) from front end using this submission form.',DOMAIN),site_url()),
				'status'=> $setup_submit_form),  			
				array('title'=>__('Create Custom fields',DOMAIN),
				'link'=> site_url().'/wp-admin/admin.php?page=custom_fields',
				'desc'=>sprintf(__('Create new <a href="%s/wp-admin/admin.php?page=custom_fields">custom fields</a> with different field types which will appear in the submission form created in the above step to get relevant information from your users on submitting their entries. We have already created some default fields (and necessary) for you.',DOMAIN),site_url()),
				'status'=>'true'),
				array('title'=>__('Create Login/Registration and Profile Pages',DOMAIN),
				'link'=>  site_url().'/wp-admin/admin.php?page=templatic_settings#registration_page_setup',
				'desc'=>sprintf(__('Registration and related process are helpful to let users register themselves with you. So, we have already created login, registration and profile pages on your site, however you can change these WordPress page to use as these forms from <a href="%s/wp-admin/admin.php?page=templatic_settings#registration_page_setup">General settings</a>.<br/> Make sure you add appropriate shortcodes in those pages,<br/> Login page: Use [tevolution_login] <br/> Registration Page:  [tevolution_register] <br/> Profile Page: [tevolution_profile]',DOMAIN),site_url()),
				'status'=>$user_reg_mod),
				array('title'=>__('Create Profile Custom Fields',DOMAIN),
				'link'=>  site_url().'/wp-admin/admin.php?page=user_custom_fields',
				'desc'=>sprintf(__('Create <a href="%s/wp-admin/admin.php?page=user_custom_fields">profile custom fields</a> which will appear on the registration and profile pages that you have created in the above steps to get the required information from your users when they register on your site.',DOMAIN),site_url()),
				'status'=> $user_reg_mod),
				array('title'=>__('Create a Navigation Menu',DOMAIN),
				'link'=> site_url().'/wp-admin/nav-menus.php',
				'desc'=>sprintf(__('WordPress Menu makes it extremely easy to manage which links you want to show in your navigation menu. Create your menu <a href="%s/wp-admin/nav-menus.php">now</a>.',DOMAIN),site_url()),
				'status'=> $tevolution_nav_menu),
				array('title'=>__('Monetize Your Site',DOMAIN),
				'link'=> site_url().'/wp-admin/admin.php?page=monetization',
				'desc'=>sprintf(__('Charge your users for posting their entries by adding <a href="%s/wp-admin/admin.php?page=monetization">price packages</a> on submission forms. There is wide range of selection of payment gateways to charge them too.',DOMAIN),site_url()),
				'status'=> $monetization_status),
				array('title'=>__('Email Setup ',DOMAIN),
				'link'=> site_url().'/wp-admin/admin.php?page=templatic_settings&tab=email',
				'desc'=>sprintf(__('Configure all the emails which will be sent to users and also the ones you will receive as an admin. Give all your emails a personal touch <a href="%s/wp-admin/admin.php?page=templatic_settings&tab=email"><b>editing their content</b></a>.',DOMAIN),site_url()),
				'status'=> true),
				array('title'=>__(' Configure other options',DOMAIN),
				'link'=>site_url().'/wp-admin/admin.php?page=templatic_settings',
				'desc'=>sprintf(__('You can change the appearance of your site with bulk of other options we provide. <a href="%s/wp-admin/admin.php?page=templatic_settings">Explore</a> these options to learn more about it. ',DOMAIN),site_url()),
				'status'=> $other_flag));
				if($setup_steps){ ?>
...
function tevolution_extend_box(){
	...
	if( is_wp_error( $response ) ) {		 
			...
			printf(__('<strong>templatic.com connect Error</strong>: %s'), $response->get_error_message());			
		} else {
		  $data = $response['body'];
		}	
		...
		if($sourceNode){
			foreach($sourceNode as $source)
			{
				...
				$theme = wp_get_theme(get_current_theme());
					...
					<div id="templatic_<?php echo str_replace('-','',$name);?>" class="widget_div">
						  ...
						  <div class="inside">
							  ...
							  <div id="publishing-action" class="settings_style">
								<a href="<?php echo $donwload_url;?>" class="button-primary" target="_blank"><?php _e('Purchase and install',DOMAIN); ?></a>
				...
				}else if(is_plugin_active($filepath) || !is_plugin_active($filepath))
				{
					...
					?>
						...
						if(!get_option($add_query_arg)):?>                               	
                              	<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&tab=extend&activated=$add_query_arg&plugin=".$filepath."&true=1";?>" class="button-primary"><i class="fa fa-check"></i><?php _e('Activate &rarr;',DOMAIN); ?></a>
                              <?php else:?>                                   
				...
				<div id="templatic_<?php echo str_replace('-','',$name);?>" class="widget_div">
				   ...
				   <div id="publishing-action" class="settings_style">    
					...
					if(!is_plugin_active($filepath)):?>                               	
						<a href="<?php echo site_url()."/wp-admin/admin.php?page=templatic_system_menu&tab=payment-gateways&activated=$add_query_arg&plugin=".$filepath."&true=1";?>" class="button-primary"><i class="fa fa-check"></i><?php _e('Activate &rarr;',DOMAIN); ?></a>
					<?php else:?>                                   
