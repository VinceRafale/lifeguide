...
*/
	...
	define('TEVOLUTION_VERSION','2.0.5');
	...
	@define('DOMAIN','tevolution');
	
	load_textdomain( DOMAIN, plugin_dir_path( __FILE__ ).'languages/en_US.mo' );
	//load_plugin_textdomain( DOMAIN, plugin_dir_path( __FILE__ ));
	//load_plugin_textdomain( DOMAIN,false, dirname( plugin_basename( __FILE__ ) ) . '/languages/' ); it's deprecated since version 2.7
	global $templatic,$wpdb,$tevolutions_icon;
	$tevolutions_icon = array('events,listing');
			...
			$paypal_update = array(
				...
				'payOpts' => array
					(
						array
							(
								'title' => 'Your PayPal email',
								'fieldname' => 'merchantid',
								'value' => 'email@example.com',
								'description' => 'Example : email@example.com',
							),
					),			
			);
...
?>
				<div class="updated">
					<p><?php _e( "'Enable for registration form' option will not working properly with Tevolution, so simply ignore it.",DOMAIN ); ?></p>
				</div>
<?php
			}
		}	
