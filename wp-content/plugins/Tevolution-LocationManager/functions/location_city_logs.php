	...
	switch ($location_tabs):
		case 'location_city_log' :
			...
			if(isset($_REQUEST['city_id']) && $_REQUEST['city_id']!=''){
				?>
                     <h2><?php _e('City Wise Logs',LM_DOMAIN);?>	
                     <a id="country_list" href="<?php echo site_url().'/wp-admin/admin.php?page=location_settings&location_tabs=location_city_log';?>" title="<?php _e('Back to city logs',LM_DOMAIN);?>" name="btnviewlisting" class="add-new-h2" /><?php _e('Back to city logs',LM_DOMAIN); ?></a>
                     </h2>
				...
				echo '<h2>'.__('City Logs',LM_DOMAIN).'</h2>';
				?>
                    <p class="tevolution_desc"><?php _e('Use this section to see how many times was each of your cities visited. This is useful in determining which is your most popular city.',LM_DOMAIN);?></p>
...
{	
	...
	function get_columns()
	{
		$columns = array(	
			...
			'title'   =>  __('City Name',LM_DOMAIN),
			'state'   =>  __('State Name',LM_DOMAIN),
			'country' =>  __('Country Name',LM_DOMAIN),
			'count'   =>  __('Total Count',LM_DOMAIN),
			'view'    => __('Views',LM_DOMAIN),
			);
		return $columns;
	}
...
{	
	...
	function get_columns()
	{
		$columns = array(	
			'cb'      => '<input type="checkbox" />',
			'title'   =>  __('City Name',LM_DOMAIN),
			'ip_address'   =>  __('IP Address',LM_DOMAIN),
			'counter' =>  __('Counter',LM_DOMAIN),			
			);
		return $columns;
	}
