...
if( !class_exists('WPDirectoryUpdates') ) {
	...
	{		
		//check the remote version
		 ...
		 {	
			$new_version = version_compare($this->plugin_version , $remote_version, '<') ? __('There is a new version of Tevolution-Directory plugin available ', DOMAIN)  : '';
			...
			echo '<tr class="plugin-update-tr"><td colspan="3" class="plugin-update"><div class="update-message">' . $new_version . __( ' <a href="'.$ajax_url.'" class="thickbox" title="Templatic directory Update">update now</a>.', DOMAIN) .'</div></td></tr>';
