...
<div class='wrap templatic_login'>
  ...
  <div id="update-nag">
    ...
    <p>
      <?php _e('you can update to the latest version automatically , or download the latest version of the theme.',THEME_DOMAIN); ?>
    </p>
    <p><span style="color:red; font-weight:bold;">
      <?php _e('Warning',THEME_DOMAIN); ?>
      : </span>
      <?php _e('Remember that the updates will replace all your changes so please keep backup of your changes.',THEME_DOMAIN); ?>
    </p>
    <a class="button-secondary" href="http://templatic.com/members/member" target="blank">
    <?php _e('Download latest Version',THEME_DOMAIN); ?>
    </a> </div>
  <?php } ?>
  <!-- Logo -->
  ...
  <?php
		if($flag ==1){
			...
			{?>
  <div class='error'>
    <p><strong><?php echo sprintf(__('%s',THEME_DOMAIN), $warnning_message);?></strong></p>
  </div>
  <?php
			  }
			if(!isset($successfull_login) && $successfull_login!=1):?>
  <p class="info"> <?php echo  sprintf(__('Enter your templatic login credentials to update your %s theme to the latest version.',THEME_DOMAIN),$th_name);?> </p>
  <form action="<?php echo site_url()."/wp-admin/admin.php?page=tmpl_theme_update";?>" name="" method="post">
    <table>
      <tr>
        <td><label>
            <?php _e('User Name', THEME_DOMAIN)?>
          </label></td>
        <td><input type="text" name="templatic_username"  /></td>
      </tr>
      <tr>
        <td><label>
            <?php _e('Password', THEME_DOMAIN)?>
          </label></td>
