	...
	class templatic_twiter extends WP_Widget{
		function templatic_twiter(){
			...
			$widget_options = array(
				...
				'description'	=>	__('Display latest tweets from your Twitter account. Works best in sidebar areas.',THEME_DOMAIN)
			);
			$this->WP_Widget(false, __('T &rarr; Twitter Feed',THEME_DOMAIN), $widget_options);
		}
...
function templatic_twitter_messages($options){
	...
	if (empty($tweets)) {
		...
		echo sprintf(__('Unable to get tweets %s',THEME_DOMAIN),$debug);
	}else{
