...
function supreme_document_title() {
	...
	elseif ( is_archive() ) {
		...
		elseif ( is_date () ) {
			if ( get_query_var( 'minute' ) && get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', THEME_DOMAIN ), get_the_time( __( 'g:i a', THEME_DOMAIN ) ) );
			elseif ( get_query_var( 'minute' ) )
				$doctitle = sprintf( __( 'Archive for minute %1$s', THEME_DOMAIN ), get_the_time( __( 'i', THEME_DOMAIN ) ) );
			elseif ( get_query_var( 'hour' ) )
				$doctitle = sprintf( __( 'Archive for %1$s', THEME_DOMAIN ), get_the_time( __( 'g a', THEME_DOMAIN ) ) );
			elseif ( is_day() )
				$doctitle = sprintf( __( 'Archive for %1$s', THEME_DOMAIN ), get_the_time( __( 'F jS, Y', THEME_DOMAIN ) ) );
			elseif ( get_query_var( 'w' ) )
				$doctitle = sprintf( __( 'Archive for week %1$s of %2$s', THEME_DOMAIN ), get_the_time( __( 'W', THEME_DOMAIN ) ), get_the_time( __( 'Y', THEME_DOMAIN ) ) );
			elseif ( is_month() )
				$doctitle = sprintf( __( 'Archive for %1$s', THEME_DOMAIN ), single_month_title( ' ', false) );
			elseif ( is_year() )
				$doctitle = sprintf( __( 'Archive for %1$s', THEME_DOMAIN ), get_the_time( __( 'Y', THEME_DOMAIN ) ) );
		}
	...
	elseif ( is_search() )
		$doctitle = sprintf( __( 'Search results for &quot;%1$s&quot;', THEME_DOMAIN ), esc_attr( get_search_query() ) );
