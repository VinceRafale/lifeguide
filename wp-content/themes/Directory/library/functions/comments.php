...
function supreme_comment_form_args( $args ) {
	...
	$args = array(
		...
		'logged_in_as' => '<p class="log-in-out">' . sprintf( __( 'Logged in as <a href="%1$s" title="%2$s">%2$s</a>.', THEME_DOMAIN ),get_author_posts_url( get_current_user_id() ), esc_attr( $user_identity ) ) . ' <a href="' . wp_logout_url( get_permalink() ) . '" title="' . esc_attr__( 'Log out of this account', THEME_DOMAIN ) . '">' . __( 'Log out &raquo;', THEME_DOMAIN ) . '</a></p><!-- .log-in-out -->',
